...
select customer_id,account_id from accounts where balance 
in (select max(balance) from accounts);
...
(select avg(amount) as transaction_amount from transactions );

select customer_id from accounts where account_id in 
(select distinct account_id from transactions where transaction_id is null);

select balance from accounts where account_id not in
...
select distinct*from accounts where account_id in
...
(select account_id from accounts where customer_id = 123);
